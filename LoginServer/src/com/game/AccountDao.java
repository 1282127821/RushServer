...
public class AccountDao extends DBDao {
	private static final String insertSql = "insert into tbl_account(AccountId, AccountName, CreateTime, LoginIp) value(?, ?, ?, ?);";
	...
	private static final String updateLoginSql = "update tbl_account set LoginTime = ?, LoginIp = ?, LogoutTime= ? where AccountId = ?;";
	...
	public boolean updateLoginAccount(AccountInfo account) {
		...
		try {
			...
			pstmt.setString(2, account.getLoginIP());
			pstmt.setInt(3, account.getLogoutTime());
