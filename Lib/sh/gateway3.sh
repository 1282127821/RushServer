...
#JMX远程监听端口设置
JDK_OPTS=$JDK_OPTS" -Dcom.sun.management.jmxremote.port=8905 -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false"
JDK_OPTS=$JDK_OPTS" -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8805"
#GC日志
...
JDK_OPTS=$JDK_OPTS" -XX:+PrintHeapAtGC -Xloggc:/game/server/app/gateway3/log/gc.log"

APPLICATION_NAME=gateway3
MAIN_CLASS="com.star.light.GatewayServer"

EXECUTABLE=/game/server/app/kernal.sh
exec "$EXECUTABLE" "$JDK_OPTS" "$APPLICATION_NAME" "$MAIN_CLASS" 2 2 "$@" "$@"
