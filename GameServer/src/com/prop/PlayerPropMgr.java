...
public class PlayerPropMgr
{
	...
	public void loadFromDB()
	{
		...
		{
			// ItemChangeListMsg.Builder itemChangeList =
			// ItemChangeListMsg.newBuilder();
	...
	public boolean removeProp(List<ResourceInfo> removeList, short itemChangeType)
	{
		...
		for (int i = 0; i < removeList.size(); i++)
		{
			...
			{
				Log.error("removePropByPropId多个 error. userId : " + player.getUserId() + " , propId : " + info.resourceId + ", castCount : " + info.count
						+ ", inventoryType : " + inventoryType + ", bagCount : " + bagCount + ", itemChangeType : " + itemChangeType);
				return false;
			}
		}
	...
	public boolean removePropByPropId(int propId, int castCount, short itemChangeType)
	{
		...
		if (!isSuccess)
		{
			Log.error("removePropByPropId error. userId : " + player.getUserId() + " , propId : " + propId + " , castCount : " + castCount + " , bagType : "
					+ bagType + ", itemChangeType : " + itemChangeType);
		}
	...
	public boolean removePropByPropId(int inventoryType, int[] propIds, int[] castCounts, short itemChangeType)
	{
		...
		if (propInventory != null)
		{
			for (int i = 0; i < propIds.length; i++)
			{
				...
				{
					Log.error("removePropByPropId多个 error. userId : " + player.getUserId() + " , propId : " + propIds[i] + " , castCount : " + castCounts[i]
							+ " , inventoryType : " + inventoryType + "bagCount " + bagCount + ", itemChangeType : " + itemChangeType);
					return false;
				}
			}
	...
	/**
	 * 往存储空间里面添加一个物品, 外部需要保证这里的数量保证传进来是不能够大于自身的堆叠数量。有需要添加超过自身堆叠数量的， 请外部逻辑自行处理
	 * 另外这个调用创建后的道具或者装备一定是保证能够添加到存储空间的，统计的时候需要用到。
	 */
	public Prop addOnePropToPackage(int propId, int count, short addType)
	{
		...
		newPropInstance.setGainTime(TimeUtil.getSysCurSecond());
	...
	{
		int curSeconds = TimeUtil.getSysCurSecond();
