...
import com.star.light.action.guild.GuildDeleteAction;
import com.star.light.action.guild.SyncGuildMemberInfo;
import com.star.light.db.DBOption;
import com.star.light.execaction.Action;
import com.star.light.player.DaoMgr;
import com.star.light.player.GamePlayer;
import com.star.light.player.PlayerSynchType;
...
import tbgame.pbmessage.GamePBMsg.GuildAcceptApplyMsg;
import tbgame.pbmessage.GamePBMsg.GuildCommonMsg;
import tbgame.pbmessage.GamePBMsg.GuildInfoMsg;
import tbgame.pbmessage.GamePBMsg.GuildListMsg;
import tbgame.pbmessage.GamePBMsg.GuildMsg;
...
public final class GuildMgr {
	...
	public void dismissGuild(long guildId, GamePlayer player) {
		...
		GuildMgr.getInstance().enDefaultQueue(new GuildDeleteAction(guildId));
		GuildCommonMsg.Builder netMsg = GuildCommonMsg.newBuilder();
		netMsg.setGuildId(-1);
	...
	public void syncGuildMemInfo(long guildId, long userId, short propertyType, int propertyValue) {
		Guild guild = getGuildById(guildId);
		if (guild != null) {
			guild.enqueue(new SyncGuildMemberInfo(guild, userId, propertyType, propertyValue));
		}
	}
