...
import com.pbmessage.GamePBMsg.EquipBlessMsg;
...
import com.prop.EquipBlessResult;
...
import com.table.EquipBlessEffectInfo;
import com.table.EquipBlessEffectInfoMgr;
import com.table.EquipBlessInfo;
...
public class EquipBlessCmd implements NetCmd {
	public void execute(GamePlayer player, PBMessage packet) throws Exception {
		...
		if (propMgr == null) {
			Log.error("装备祝福出错, PlayerPropMgr is NULL." + ", UserId: " + player.getUserId());
			return;
		}

		PropInventory equipInventory = propMgr.getPropInventory(bagType);
		if (equipInventory == null) {
			Log.error("装备祝福出错, equipInventory is NULL." + ", UserId: " + player.getUserId());
			return;
		}

		PropInventory propInventory = propMgr.getPropInventory(BagType.PACKAGE);
		if (propInventory == null) {
			Log.error("装备祝福出错, propInventory为空." + ", UserId: " + player.getUserId());
			return;
		}
		
		Prop equip = equipInventory.getPropByPosIndex(posIndex);
		if (equip == null) {
			Log.error("装备祝福出错, 道具为空，位置为:  " + posIndex +  ", UserId: " + player.getUserId());
			return;
		}
		
		int masterType = equip.getMasterType();
		if (equip.getMasterType() != PropType.EQUIP) {
			Log.error("装备祝福出错, 此道具不是装备 MasterType:  " + masterType +  ", UserId: " + player.getUserId());
			return;
		}
		...
		if (isForbidden) {
			...
			Log.error("装备祝福出错, 该装备不能够被祝福，equipId为:  " + equipId + ", UserId: " + player.getUserId());
			return;
		}
		...
		if (srcBlessLv >= ConfigMgr.equipBlessMaxLv) {
			...
			Log.error("装备祝福出错, 找不到对应的祝福材料，装备当前祝福等级: " + srcBlessLv + ", UserId: " + player.getUserId());
			return;
		}
		...
		if (equipBlessInfo == null) {
			Log.error("装备祝福出错, 找不到对应的祝福材料，装备品级: " + equipQuality + ",装备当前祝福等级: " + srcBlessLv + ", UserId: " + player.getUserId());
			return;
		}
		...
		if (equipBlessEffectInfo == null) {
			Log.error("装备祝福出错, 找不到对应的装备效果，装备品级: " + equipQuality + ", UserId: " + player.getUserId());
			return;
		}
		...
		if (costGold > haveGold) {
			Log.error("装备祝福出错, 所需金钱: " + costGold + ",当前金钱: " + haveGold + ", UserId: " + player.getUserId());
			return;
		}
		...
		if (needCount == 0) {
			Log.error("装备祝福出错, 使用的祝福材料不对 blessMaterialId: " + blessMaterialId + ", UserId: " + player.getUserId());
			return;
		}
		
		int haveCount = propInventory.getSpecificPropCount(blessMaterialId);
		if (needCount > haveCount) {
			Log.error("装备祝福出错, 材料数量不对 所需材料数量: " + needCount + ",当前拥有材料数量: " + haveCount + ", UserId: " + player.getUserId());
			return;
		}
